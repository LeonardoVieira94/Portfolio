...
class Papainoel:
    ...
    def movimento(self):
        self.tempoJogo += 1
        aceleracao = 3
        deslocamentoPersonagem = self.velocidadePersonagem * self.tempoJogo + (aceleracao * (self.tempoJogo**2))/2
...
class Obstaculo:
    ...
    def escolher_obstaculo(self):
        global ob1, ob2
        self.proximoObstaculo = random.randint(1,2)
        if self.proximoObstaculo == 1:
            ob2 = 0
            ob1 += 1
            if ob1 <= 3:
                ...
                self.proximoObstaculo = 2
                ob2 += 1
                ob1 = 0
        elif self.proximoObstaculo == 2:
            ob1 = 0
            ob2 += 1
            if ob2 <= 3:
                self.alturaObstaculo = 55
    ...
    while vivo:
        ...
        for obstaculo in obstaculos:
            ...
            if obstaculo.posx + obstaculo.obstaculoBaixo.get_width() < 0:
                removerObstaculo.append(obstaculo)
        if novoObstaculo:
            pontos +=1
        ...
        for obstaculo in removerObstaculo:
